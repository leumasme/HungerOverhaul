...
public class ModulePlantGrowth {
    ...
    public static void registerPlantGrowthModifier(Class<? extends Block> blockClass,
            PlantGrowthModification growthModification) {
    ...
    public static PlantGrowthModification getPlantGrowthModification(Class<? extends Block> blockClass) {
        ...
        if (growthModifier == null) {
            for (Map.Entry<Class<? extends Block>, PlantGrowthModification> entry : plantGrowthModificationsByBlockClass
                    .entrySet()) {
    ...
    public void allowGrowthTick(PlantGrowthEvent.AllowGrowthTick event) {
        ...
        float sunlightModifier = !growthModification.needsSunlight
                || (growthModification.needsSunlight && event.world.isDaytime()
                        && event.world.canBlockSeeTheSky(event.x, event.y, event.z)) ? 1
        ...
        // random
        if (RandomHelper
                .nextFloat(event.random, growthModification.growthTickProbability * biomeModifier * sunlightModifier)
                >= 1) {
            event.setResult(Result.DENY);
            return;
        }
