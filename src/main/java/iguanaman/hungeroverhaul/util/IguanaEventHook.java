...
public class IguanaEventHook {
    ...
    public void onRenderGameOverlay(RenderGameOverlayEvent.Text event) {
        if (IguanaConfig.addGuiText) {
            ...
            if (!player.isDead && !player.capabilities.isCreativeMode && !mc.gameSettings.showDebugInfo) {
                ...
                if (healthPercent <= 0.15F) event.left.add(
                        EnumChatFormatting.RED + StatCollector.translateToLocal("hungeroverhaul.dying")
                                + EnumChatFormatting.RESET);
                else if (healthPercent <= 0.3F) event.left.add(
                        EnumChatFormatting.YELLOW + StatCollector.translateToLocal("hungeroverhaul.injured")
                                + EnumChatFormatting.RESET);
                else if (healthPercent < 0.5F) event.left.add(
                        EnumChatFormatting.WHITE + StatCollector.translateToLocal("hungeroverhaul.hurt")
                                + EnumChatFormatting.RESET);

                if (player.getFoodStats().getFoodLevel() <= 6) event.right.add(
                        EnumChatFormatting.RED + StatCollector.translateToLocal("hungeroverhaul.starving")
                                + EnumChatFormatting.RESET);
                else if (player.getFoodStats().getFoodLevel() <= 10) event.right.add(
                        EnumChatFormatting.YELLOW + StatCollector.translateToLocal("hungeroverhaul.hungry")
                                + EnumChatFormatting.RESET);
                else if (player.getFoodStats().getFoodLevel() <= 14) event.right.add(
                        EnumChatFormatting.WHITE + StatCollector.translateToLocal("hungeroverhaul.peckish")
                                + EnumChatFormatting.RESET);
            }
        }
    }
    ...
    public void onEntityInteractEvent(EntityInteractEvent event) {
        if (IguanaConfig.milkedTimeout > 0 && event.entityPlayer != null
                && event.target != null
            ...
            if (equipped != null && equipped.getItem() != null) {
                ...
                if (item instanceof ItemBucket && ((ItemBucket) item).isFull == Blocks.air
                    ...
                    if (tags.hasKey("Milked")) {
                        ...
                        if (!player.worldObj.isRemote) cow.playSound(
                                "mob.cow.hurt",
                                0.4F,
    ...
    public void onPlayerInteraction(PlayerInteractEvent event) {
        ...
        if (IguanaConfig.foodsUnplantable && Loader.isModLoaded("harvestcraft")
        ...
        if (resultingMeta >= 0) {
            ...
            if (!event.world.isRemote && !event.world.restoringBlockSnapshots) // do not drop items while restoring
            ...
            if (adjective != null && StatCollector.canTranslate("hungeroverhaul." + adjective + "." + noun)) {
                ...
                if (adjective != null) {
                    mealDescriptor = StatCollector.translateToLocalFormatted(
                            StatCollector.translateToLocal("hungeroverhaul." + adjective),
                            mealDescriptor);
                }
            }
    ...
    private boolean isWaterNearby(World par1World, int par2, int par3, int par4) {
        for (int l = par2 - 4; l <= par2 + 4; ++l)
            for (int i1 = par3; i1 <= par3 + 1; ++i1) for (int j1 = par4 - 4; j1 <= par4 + 4; ++j1)
